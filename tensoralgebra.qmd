...
$|\bfv|\geq0$. The zero vector, $\bf0$, is the unique vector with zero
...
$\bfu\times\bfv=\bf0$, then $\bfu$ and $\bfv$ are *parallel*. Recall
...
$\bfO(\bfv) = \bf0$ for all $\bfv\in\calV$, and the identity tensor
...
### Tensor products, bases and the scalar product
...
It turns out that the basis defined in this way is actually an orthonormal basis.
Just as for vectors, we can define a *scalar product* for second-order
tensors, denoted
$$
  \bfS:\bfD = \tr(\bfS^T\bfD).
...
$\{\bfe'_i\}$, we mean
$$\bfA = A_{ij}\bfe_i\otimes \bfe_j\quad\text{where}\quad A_{ij} = \bfe_i\cdot\bfe'_j.$$
We could equally define a change of basis tensor $\bfB$ from
$\{\bfe'_i\}$ to $\{\bfe_i\}$, defined to be
$\bfB = B_{ij}\bfe'_i\otimes\bfe'_j$ with $B_{ij} = \bfe'_i\cdot\bfe_j$,
...
$$ {#eq-changeofbasis1}
  Similarly,
working with respect to the second basis, we have that
$$
...
$$
  [\bfS] = [\bfA][\bfS]'[\bfA]^T\quad\text{and}\quad [\bfS]'=[\bfA]^T[\bfS][\bfA].
