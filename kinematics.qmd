...
A deformation $\bfphi:B\to B'$ is said to be **admissible** if:
...
:::
 ...
 The first of these assumptions means that two
...
One way to quantify strain is by taking the derivative of the deformation:

:::{.callout-note icon="false"}
## Deformation gradient
The **deformation gradient** is a
second-order tensor field $\bfF:B\to\calV^2$ defined to be
$$\bfF(\bfx) = \nabla \bfphi(\bfx).$$ 
:::

The field $\bfF$ naturally provides
...
To understand better how $\bfF$ measures strain, we will look at the following simple class of deformations.

::: {.callout-note icon="false"}
## Homogeneous deformation
A deformation $\bfphi$ is **homogeneous** if its deformation gradient field
is constant, i.e. it can be written as
$$
\bfphi(\bfx) = \bfc+ \bfF\bfx
$$
for a constant vector $\bfc\in\calV$ and a constant tensor $\bfF\in\calV^2$.
:::

In other words, homogeneous deformations are **affine maps**. We note that by virtue of this affine nature, any line
...
### Translations, Rotations and Rigid body deformations

We now define some special classes of homogeneous deformation.
...
Under the action of a translation, all points in $B'$ are simply moved through a displacement $\bfc$, so there is no change in shape or orientation of the body.

::: {.callout-note icon="false"}
## Rotation
A homogeneous deformation $\bfphi$ is called a **rotation about $\bfx'$** if
it takes the form $$\bfphi(\bfx)=\bfx'+\bfQ(\bfx-\bfx')$$ for some
rotation tensor $\bfQ\in\calV^2$.
:::

As for translations, rotations change the orientation of the body, but not its shape. Together, we can combine these two classes to get:

::: {.callout-note icon="false"}
## Rigid deformation
A homogeneous deformation $\bfphi$ is called a **rigid deformation** if it can be written as $$\bfphi(\bfx) = \bfc+\bfQ\bfx$$ where $\bfc\in\calV$ and $\bfQ\in\calV^2$ is a rotation tensor.
:::

Rigid deformations are important since they correspond to moving a body in three dimensions with changing its shape. Similarly, they encode important symmetries of Euclidean space, allowing us to change the origin and orthonormal basis of our coordinate system.

Looking back at our definition of what it means to be a rotation about a point $\bfx'$, we see that it makes sense to define $\bfx'$ as a fixed point of the deformation, since $\bfphi(\bfx') = \bfx'$. This gives us the following definition:
...
For a deformation with a fixed point, any point which is not $\bfx'$ in the body is displaced by an amount determined by $\bfF$ and the position relative to the fixed point $\bfx'$. For such a deformation, the body can change both shape and orientation, and we now define some classes of homogeneous deformation with fixed points.

::: {.callout-note icon="false"}
## Stretch
A homogeneous deformation $\bfphi$ is called a **stretch about $\bfx'$** if
it takes the form $$\bfphi(\bfx)=\bfx'+\bfS(\bfx-\bfx')$$ for some
symmetric positive definite tensor $\bfS\in\calV^2$. 
:::
When undergoing a stretch, the orientation of the body is not changed, but it is
...
point** at $\bfx'$. In particular, $\bfg$ takes the form
$$
\bfg(\bfx) = \bfx'+\bfF(\bfx-\bfx').
$$
:::

The following result shows that an arbitrary homogeneous deformation with a fixed point can always be decomposed as a rotation and a stretch about the same fixed point, using the polar decomposition theorem.
...
::: {.callout-note icon="false"}
## Extension
A homogeneous deformation $\bfphi$ is called an **extension**
about $\bfx'$ in the direction of the unit vector $\bfe$ if
$$\bfphi(\bfx) = \bfx'+\bfF(\bfx-\bfx')\quad\text{with}\quad \bfF=\bfI+(\lambda-1)\bfe\otimes\bfe,$$
for some $\lambda>0$.
:::
 This terminology is based on the observation that
...
We make a few notes about this result:

-  The tensors $\bfU$ and $\bfV$ which appear in the right and left
...
-  We call the eigenvalues $\lambda_i$ the **principal stretches**
...
A second measure of strain is:

::: {.callout-note icon="false"}
## Cauchy-Green strain
The **(right) Cauchy-Green strain tensor** field is $\bfC:B\to\calV^2$, defined by
$$\bfC = \bfF^T\bfF.$$
:::

Notice that $\bfC$ is symmetric and positive definite at every point in $B$. While $\bfF$ contains information on both rotation and stretching,
$\bfC$ includes information about stretches only. Since
...
We now consider the interpretation of $\bfC$. Consider an arbitrary point $\bfx_0$ in the reference configuration, and let $\Omega$ be the open ball of radius $\alpha>0$ centred at $\bfx_0$. Consider the points
$$
\bfx' = \bfx_0+\alpha\bfe'\quad\text{and}\quad\bfx''=\bfx_0+\alpha\bfe'',
$$
where $\bfe'$ and $\bfe''$ are unit vectors. Let
$$
\bfy_0=\bfphi(\bfx_0),\quad
\bfy'=\bfphi(\bfx')\quad\text{and}
\quad\bfy''=\bfphi(\bfx'')
$$
be the corresponding points in the deformed configuration. Let $\phi\in[0,\pi]$ be the angle
between the displacement vectors $\bfy'-\bfy_0$ and $\bfy''-\bfy_0$ in the deformed configuration. Then we have the following result:

::: {#prp-CGstrainrelations}
For any point $\bfx_0\in B$ and unit vectors $\bfe'$ and $\bfe''$, define
$$\lambda(\bfe') = \sqrt{\bfe'\cdot\bfC\bfe'}>0 \quad\text{and}\quad
    \theta(\bfe',\bfe'') = \arccos\bigg(\frac{\bfe'\cdot\bfC\bfe''}{\lambda(\bfe')\lambda(\bfe'')}\bigg)\in[0,\pi].$$
Then, as $\alpha\to0$, we have that
$$\frac{|\bfy'-\bfy_0|}{|\bfx'-\bfx_0|}\to\lambda(\bfe'),\quad \frac{|\bfy''-\bfy_0|}{|\bfx''-\bfx_0|}\to\lambda(\bfe'')$$
and $$\phi\to \theta(\bfe',\bfe'').$$
:::

::: proof
Note that $\bfy'-\bfy_0 = \bfphi(\bfx_0+\alpha\bfe')-\bfphi(\bfx_0)$, so
Taylor expanding, we have
$$\bfy'-\bfy_0 = \alpha \bfF(\bfx_0)\bfe'+\bigO(\alpha^2).$$ It follows that
$$|\bfy'-\bfy_0|^2 = \alpha^2\Big(\bfF(\bfx_0)\bfe'\Big)\cdot\Big(\bfF(\bfx_0)\bfe'\Big)+\bigO(\alpha^3) = \alpha^2 \bfe'\cdot\bfC(\bfx_0)\bfe'+\bigO(\alpha^3).$$
Dividing through by $\alpha^2=|\bfx'-\bfx_0|^2$, we have
$$
\frac{|\bfy'-\bfy_0|^2}{|\bfx'-\bfx_0|^2}
 = \bfe'\cdot\bfC(\bfx_0)\bfe' +\bigO(\alpha),$$
...
$$\cos\phi = \frac{(\bfy'-\bfy_0)\cdot(\bfy''-\bfy_0)}{|\bfy'-\bfy_0||\bfy''-\bfy_0|}.$$ Using the facts that
$\bfy'-\bfy_0=\alpha\bfF(\bfx)\bfe'+\bigO(\alpha^2)$ and
$\bfy''-\bfy_0=\alpha\bfF(\bfx)\bfe''+\bigO(\alpha^2)$, and using the first part of the result, we have
$$
\cos\phi = \bfe'\cdot\bfC(\bfx)\bfe''+\bigO(\alpha).
$$
Letting $\alpha\to0$, we conclude.
:::

**Remarks:**

-  This result states that $\lambda(\bfe)$ approximates the ratio of
...
-  At any fixed reference point $\bfx$, the extreme values of
...
-  $\theta(\bfe',\bfe'')$ approximates the angle between points in the
    deformed configuration which lie at short distances from $\bfx$
    along the directions $\bfe'$ and $\bfe''$. If
    $\Theta(\bfe',\bfe'') = \arccos(\bfe'\cdot\bfe'')$ is the angle between
    these directions in the reference configuration, we define
    $$\gamma(\bfe',\bfe'') = \Theta(\bfe',\bfe'')-\theta(\bfe',\bfe''),$$ which
    we call the **shear** between the directions $\bfe'$ and $\bfe''$ at the point $\bfx$. The shear measures the change in angle between directions due to deformation.

The following result shows that $\bfC$ explicitly characterises the
stretch and shear caused by a deformation when considered in a particular coordinate frame.
...
To finish our discussion of the Cauchy-Green strain, we note that if $\bfphi$ is a rigid deformation, we have $\bfF=\bfQ$ where $\bfQ$ is a rotation tensor, and
...
The **infinitesimal strain tensor**
field associated with a deformation is $\bfeps:B\to\calV^2$ defined by
$$\bfeps = \sym(\nabla\bfu) = \tfrac12(\nabla\bfu+\nabla\bfu^T).$$
:::

Note that, by definition, $\bfeps$ is symmetric. $\bfeps$ can related to the deformation gradient $\bfF$ and the
Cauchy-Green tensor $\bfC$. From the definition of $\bfu$, we see that
$\nabla \bfu = \bfF-\bfI$, and hence $\bfeps = \sym(\bfF-\bfI)$. Since
$\bfC = \bfF^T\bfF$, we see that
$$\bfeps = \tfrac12(\bfC-\bfI)-\tfrac12\nabla\bfu^T\nabla \bfu.$$ The
tensor $\bfeps$ is particularly useful in the case of small deformations.
A deformation $\bfphi$ induces **small strains** if the displacement gradient is small, i.e. $|\nabla\bfu(\bfx)|\ll 1$ for all points $\bfx\in B$.
In this case, we see that that
$$\bfeps = \tfrac12(\bfC-\bfI)+\bigO\left(|\nabla\bfu|^2\right).$$ If we drop the latter terms, then $\bfeps$ is equivalent to $\bfC$ up to a multiplicative factor and offset, and $\bfeps$ will arise when studying
...
-   Note that if $\nabla \bfu =\bfO$ everywhere in $B$, we have
    ...
    pure *translation* (not a rigid body motion!).

-   For small deformations, the tensor $\bfeps$ contains similar
    information to $\bfC$. However, we note that $\bfeps$ depends linearly
    on $\bfu$ (and hence on $\bfy$), whereas $\bfC$ depends non-linearly
    on $\bfu$.

To interpret $\bfeps$, we have the following result.

::: {#prp-Ecomponents}
Let $\varepsilon_{ij}$ be the components of $\bfeps$ taken with respect to a Cartesian frame
$\{\bfe_i\}$. Then, with no sum over repeated indices implies, we have
$$\varepsilon_{ii} =\lambda(\bfe_i)-1+\bigO(|\nabla\bfu|^2)\quad\text{and}\quad
    \varepsilon_{ij} =\tfrac12\sin\gamma(\bfe_i,\bfe_j)+\bigO(|\nabla\bfu|^2),$$
...
The result above can be proven using
@prp-stretchshear.
...
-   When the shear angle $\gamma(\bfe_i,\bfe_j)$ is small, we have that
    $$\varepsilon_{ij}\approx \tfrac12\sin\gamma(\bfe_i,\bfe_j)\approx \tfrac12\gamma(\bfe_i,\bfe_j),$$
    so for small deformations, the off-diagonal components of $\bfeps$ are
...
infinitesimal strain tensor $\bfeps$ to study $\bfL$, which leads us to
...
$\widehat\bfeps$ and of the right stretch tensor $\widehat\bfU$.
...
A motion $\bfphi$ is **rigid** if it is a rigid deformation at each time $t$, i.e.
$$
\bfphi(\bfx,t) = \bfc(t)+\bfQ(t)\bfx
$$ for some time-dependent vector
$\bfc(t)$ and rotation tensor $\bfQ(t)$. For such motions it can be
shown that the spatial velocity field takes the form
$$\bfv(\bfy,t) = \bfomega\times(\bfy-\bfc)+\frac{\dd\bfc}{\dd t},$$
where $\bfomega(t)$ is a time-dependent vector called the **spatial
angular velocity** of the motion. We can use
@prp-skewtensorvectorproduct to write this as
$$\bfv(\bfy,t) = \bfOmega(t)\big(\bfy-\bfc(t)\big)+\frac{\dd\bfc}{\dt}(t),$$ where
