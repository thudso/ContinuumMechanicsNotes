...
describe the mapping from $B$ to $B'$ by a function $\bfy:B\to B'$ which
maps each point $\bfx$ to a point $\bfy(\bfx)\in B'$. The displacement
of a material particle from its initial location $\bfx$ to final
location $\bfy(\bfx)$ is $$\bfu(\bfx) = \bfy(\bfx)-\bfx.$$ We call
$\bfu:B\to\calV$ the *displacement field* associated to $\bfy$.

We will assume that deformation maps satisfy the following two important
conditions:

-   $\bfy:B\to B'$ is one-to-one, and

-   $\det\nabla\bfy(\bfx)>0$ for all $\bfx\in B$.
...
$\bfy$ is a bijection between $B$ and $B'$.
...
within the body we consider. Under the deformation $\bfy$, $\bfx_0$ is
mapped to $\bfy_0=\bfy(\bfx_0)$, and $\Omega_\delta$ is mapped to a
region $\Omega'=\bfy(\Omega)$. Any difference in shaped between
$\Omega'$ and $\Omega$ as $\delta\to0$ is called *strain* at $\bfx_0$.
Strain refers to the local stretching of a body induced by the
deformation $\bfy$. The concept of strain plays a central roles in
...
$$\bfF(\bfx) = \nabla \bfy(\bfx).$$ The field $\bfF$ naturally provides
information on the local behaviour of $\bfy$, since Taylor expanding, we
see
$$\bfy(\bfx) = \bfy(\bfx_0) +\bfF(\bfx_0)(\bfx-\bfx_0)+\bigO\big(|\bfx-\bfx_0|^2\big),$$
or equivalently
$$\bfy(\bfx) = \bfc+\bfF(\bfx_0)\bfx+\bigO\big(|\bfx-\bfx_0|^2\big)\quad\text{where}\quad \bfc = \bfy(\bfx_0)-\bfF(\bfx_0)\bfx_0.$$

To understand better how $\bfF$ measures strain, suppose for simplicity
that $\bfy$ is *homogeneous*, which means the deformation gradient field
is constant, i.e. $$\bfy(\bfx) = \bfc+ \bfF\bfx,$$ so the deformation is
...
deformations imply that $\det\nabla\bfy(\bfx)>0$, which ensures that
$\det\bfF>0$, so $\bfF$ is invertible.

#### Translations and fixed points

A homogeneous deformation $\bfy$ is called a translation if $\bfF=\bfI$,
i.e. $$\bfy(\bfx) = \bfx+\bfc\quad\text{for some fixed }\bfc\in\calV.$$
...
$$
  \bfy(\bfx) = \bfx'+ \bfF(\bfx-\bfx').
$$ {#eq-defFixedPoint}
The point $\bfx'$ is fixed in
the sense that $\bfy(\bfx') = \bfx'$. Any other point in the body is
...
A homogeneous deformation $\bfy$ is called a *rotation about $\bfx'$* if
it takes the form $$\bfy(\bfx)=\bfx'+\bfQ(\bfx-\bfx')$$ for some
rotation tensor $\bfQ\in\calV^2$. Such deformations change the
orientation of the body, but not its shape.

A homogeneous deformation $\bfy$ is called a *stretch about $\bfx'$* if
it takes the form $$\bfy(\bfx)=\bfx'+\bfS(\bfx-\bfx')$$ for some
...
::: proposition
Let $\bfy$ be a homogeneous deformation. Then, given any point
$\bfx'\in\bbE^3$, we can decompose $\bfy$ as
$$\bfy = \bft_1\circ \bfg=\bfg\circ\bft_2,$$ where $\bft_1$ and $\bft_2$
...
If $\bfy$ is a homogeneous deformation with a fixed point $\bfx'$ and
deformation gradient $\bfF$, let $\bfF = \bfR\bfU=\bfV\bfR$ be the right
and left polar decompositions of $\bfF$. Then $\bfy$ can be decomposed
as $$\bfy = \bfr\circ \bfs_1=\bfs_2\circ\bfr,$$ where $\bfr$ is a
...
definition. A homogeneous deformation $\bfy$ is called an *extension*
about $\bfx'$ in the direction of a unit vector $\bfe$ if
$$\bfy(\bfx) = \bfx'+\bfF(\bfx-\bfx')\quad\text{with}\quad \bfF=\bfI+(\lambda-1)\bfe\otimes\bfe,$$
...
models, especially since a Taylor expansion of the deformation $\bfy$
allows us to treat deformations close to a given point as approximately
homogeneous.

### The Cauchy--Green strain tensor

Consider a general deformation $\bfy:B\to B'$ with deformation gradient
$\bfF = \nabla \bfy$. Another measure of strain is the *right
...
$\bfe$ and $\bfd$ are unit vectors. Let $\bfy_0=\bfy(\bfx_0)$,
$\bfy_1=\bfy(\bfx_1)$ and $\bfy_2=\bfy(\bfx_2)$ be the corresponding
...
::: proof
Note that $\bfv = \bfy(\bfx_0+\alpha\bfe)-\bfy(\bfx_0)$, so
...
A homogeneous deformation $\bfy:B\to B'$ is a *rigid deformation* if
$$\bfy(\bfx) = \bfc+\bfQ\bfx$$ where $\bfc\in\calV$ and $\bfQ\in\calV^2$
...
Consider a deformation $\bfy:B\to B'$ with displacement field $\bfu$ and
displacement gradient $\nabla \bfu$. The *infinitesimal strain tensor*
field associated with $\bfy$ is $\bfE:B\to\calV^2$ defined by
$$\bfE = \sym(\nabla\bfu) = \tfrac12(\nabla\bfu+\nabla\bfu^T).$$ Note
...
A deformation $\bfy$ is *small* if there is a number
...
-   Note that if $\nabla \bfu =\bfO$ everywhere in $B$, we have
    $\bfF = \bfI$, and hence $\bfy$ is a translation. It follows that
    $\bfy$ is a small deformation when it deviates only slightly from a
    pure translation.
...
A homogeneous deformation $\bfy$ is called *infinitesimally rigid* if
...
Proposition [\[prop:derivativedet\]](#prop:derivativedet){reference-type="ref"
reference="prop:derivativedet"}) is defined to be
