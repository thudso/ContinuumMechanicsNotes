...
$C>0$ and $r>0$ such that $|\bff(\bfu)|\leq C|\bfu|^r$ as $\bfu\to\bf0$,
then we write
$$\bff(\bfu) = \bigO(|\bfu|^r)\quad\text{as}\quad\bfu\to\bf0.$$ If there is a constant $r>0$ such that $$\frac{|\bff(\bfu)|}{|\bfu|^r}\to0$$ as
$\bfu\to\bf0$, then we write
$$\bff(\bfu)=\litO(|\bfu|^r)\quad\text{as}\quad\bfu\to\bf0.$$ The
...
$$\bff(\bfu) = \bfg(\bfu)+\bigO(|\bfu|^r)\quad\text{as }\bfu\to\bf0$$ to
mean that
$$\bff(\bfu) - \bfg(\bfu) = \bigO(|\bfu|^r)\quad\text{as }\bfu\to\bf0,$$
and the same for the order symbol $\litO$. We sometimes indicate the
limit $\bfu\to\bf0$ using the informal terminology 'for $\bfu$ small'.
...
directly from
@prp-gradientincoords and the component expression for the
