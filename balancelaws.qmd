...
have $$\begin{aligned}
  ...
  \frac{d}{dt}\bfl[\Omega_t] &= \sum_{i\in I}\bigg(\bff^{\text{env}}_i+\sum_{j\notin I}\bff^{\text{int}}_{ij}\bigg)\\
  \frac{d}{dt}\bfj[\Omega_t] &= \sum_{i\in I}\bfx_i\times \bigg(\bff^{\text{env}}_i+\sum_{j\notin I}\bff^{\text{int}}_{ij}\bigg)\\
  \frac{d}{dt}\big(U[\Omega_t]+K[\Omega_t]\big) &= \sum_{i\in I} \dot{\bfx}_i\cdot \bigg(\bff^{\text{env}}_i+\sum_{j\notin I}\bff^{\text{int}}_{ij}\bigg).
...
::: axiom
[]{#ax:ConservationOfMass label="ax:ConservationOfMass"}
[]{#ax:massconservation label="ax:massconservation"} The mass of any
open subset of a continuum body is invariant as the body changes
position and shape, i.e.
$$\frac{d}{dt}\mass[\Omega_t] = 0\quad\text{for all }\Omega_t\subseteq B_t.$$
:::
...
::: axiom
[]{#ax:LawsOfInertia label="ax:LawsOfInertia"} []{#ax:lawsofinertia
label="ax:lawsofinertia"} The rate of change of linear momentum of any
open subset of a continuum body equals the resultant force applied to
it, and the rate of change of angular momentum equals the resultant
torque, i.e.
$$\frac{d}{dt}\bfl[\Omega_t] = \bfr[\Omega_t]\quad\text{and}\quad \frac{d}{dt}\bfj[\Omega_t;\bfz]=\bftau[\Omega_t;\bfz]\qquad\text{for all }\Omega_t\subseteq B_t.$$
:::

Both
Axiom [\[ax:massconservation\]](#ax:massconservation){reference-type="ref"
reference="ax:massconservation"} and
Axiom [\[ax:lawsofinertia\]](#ax:lawsofinertia){reference-type="ref"
reference="ax:lawsofinertia"} are assumed to hold for every motion and
...
Chapter [\[cha:mass-forces\]](#cha:mass-forces){reference-type="ref"
reference="cha:mass-forces"}), we assume that the net heating can be
...
and arguments to those deployed in
Propositions [\[prop:action+reaction\]](#prop:action+reaction){reference-type="ref"
reference="prop:action+reaction"} and
[\[prop:stresstensor\]](#prop:stresstensor){reference-type="ref"
reference="prop:stresstensor"} show that heat transfer must take this
form.
...
$$W[\Omega_t] = P[\Omega_t]-\frac{d}{dt}K[\Omega_t].$$ It
...
::: axiom
[]{#ax:1stLaw label="ax:1stLaw"} The rate of change of the internal
...
no limit on the rate at which heat can be released.
...
[\[eq:heatinglimit\]](#eq:heatinglimit){reference-type="eqref"
reference="eq:heatinglimit"} becomes $$\label{eq:ClausiusPlanck}
...
[\[eq:ClausiusPlanck\]](#eq:ClausiusPlanck){reference-type="eqref"
reference="eq:ClausiusPlanck"} to spatially inhomogeneous systems is
then the following.

::: axiom
[]{#ax:2ndLaw label="ax:2ndLaw"} The rate of entropy production in any
open subset of a continuum body is bounded below by the heating per unit
temperature, i.e.
$$\frac{d}{dt}H[\Omega_t] \geq \int_{\Omega_t}\frac{\rho(\bfy,t)r(\bfy,t)}{\theta(\bfy,t)}\dVy-\int_{\partial \Omega_t}\frac{\bfq(\bfy,t)\cdot\bfn(\bfy)}{\theta(\bfy,t)}\dAy$$
...
### Integral versus local balance laws
...
Proposition [\[prop:localisation\]](#prop:localisation){reference-type="ref"
reference="prop:localisation"}, to derive partial differential equations
...
$\bfphi(\Omega,t) = \Omega_t$.
Axiom [\[ax:ConservationOfMass\]](#ax:ConservationOfMass){reference-type="ref"
reference="ax:ConservationOfMass"} entails that
...
$$\mass[\Omega_0] = \int_{\Omega}\rho(\bfx,0)\dVy = \int_\Omega \rho_0(\bfx)\dVx,$$
...
entails that $$\label{eq:ConsOfMass1}
...
$$\bigg(\frac{d}{dt}\rho(\bfphi(\bfx,t),t)\bigg)\det\bfF(\bfx,t)+\rho(\bfphi(\bfx,t),t)\frac{d}{d t}\det\bfF(\bfx,t) = 0.$$
For the latter term, we apply
Proposition [\[prop:TotalDerivativeDetF\]](#prop:TotalDerivativeDetF){reference-type="ref"
reference="prop:TotalDerivativeDetF"}, so that
$$\bigg(\frac{d}{dt}\rho(\bfphi(\bfx,t),t)\bigg)\det\bfF(\bfx,t)+\rho(\bfphi(\bfx,t),t)\det\bfF(\bfx,t)\nabla_\bfy\cdot\bfv(\bfphi(\bfx,t),t) = 0.$$
...
$$\frac{\partial \rho}{\partial t} +\nabla_\bfy\cdot(\rho\bfv) = 0$$ for
all $\bfy\in B_t$ and $t\in[0,T]$. Equivalently, we have

$$\DDt\rho +\rho\nabla_\bfy\cdot\bfv = 0$$ for all $\bfy\in B_t$ and
$t\in[0,T]$.
:::
...
::: proposition
[]{#prop:TimeDerivativeMassIntegrals
label="prop:TimeDerivativeMassIntegrals"} If
$\bfphi:B\times[0,T]\to\bbE^3$ is a motion with associated spatial
...
then
$$\frac{d}{dt}\int_{\Omega_t}\Phi(\bfy,t)\rho(\bfy,t)\dVy =\int_{\Omega_t}\DDt{\Phi}(\bfy,t)\rho(\bfy,t)\dVy.$$
:::

::: proof
*Proof.* Applying
Proposition [\[prop:TransformationOfVolumeIntegrals\]](#prop:TransformationOfVolumeIntegrals){reference-type="ref"
reference="prop:TransformationOfVolumeIntegrals"}, we have
...
[\[eq:ConsOfMass1\]](#eq:ConsOfMass1){reference-type="eqref"
reference="eq:ConsOfMass1"}, i.e.
$\rho(\bfphi(\bfx,t),t)\det\bfF(\bfx,t) = \rho_0(\bfx)$, we can rewrite
the RHS as
...
$$\frac{d}{dt}\int_{\Omega_t}\Phi(\bfy,t)\rho(\bfy,t)\dVy = \int_\Omega \frac{d}{dt}\bigg(\Phi(\bfphi(\bfx,t),t)\bigg)\rho_0(\bfx)\dVx.$$
But now, using equality
[\[eq:ConsOfMass1\]](#eq:ConsOfMass1){reference-type="eqref"
reference="eq:ConsOfMass1"} again, and then applying the definition of
the total derivative and
Proposition [\[prop:TransformationOfVolumeIntegrals\]](#prop:TransformationOfVolumeIntegrals){reference-type="ref"
reference="prop:TransformationOfVolumeIntegrals"}, we have
$$\begin{aligned}
    \DDt{}\int_{\Omega_t}\Phi(\bfy,t)\rho(\bfy,t)\dVy
    &= \int_\Omega \frac{d}{dt}\bigg(\Phi(\bfphi(\bfx,t),t)\bigg)\rho(\bfphi(\bfx,t),t)\det\bfF(\bfx,t)\dVx\\
...
As stated in
Axiom [\[ax:LawsOfInertia\]](#ax:LawsOfInertia){reference-type="ref"
reference="ax:LawsOfInertia"}, we have
...
of Chapter [\[cha:mass-forces\]](#cha:mass-forces){reference-type="ref"
reference="cha:mass-forces"} state that the traction field
$\bft=\bfS\bfn$, so that
...
::: proposition
[]{#prop:LinearMomentumEulerian label="prop:LinearMomentumEulerian"} If
$\bfphi:B\times[0,T]\to\bbE^3$ is a motion with associated spatial
velocity field $\bfv$ and spatial mass density field $\rho$, then the
...
Proposition [\[prop:staticequilibriumeqns\]](#prop:staticequilibriumeqns){reference-type="ref"
reference="prop:staticequilibriumeqns"}: the static result corresponds
to the case where $\dot\bfv=0$ for all $\bfy\in B_t$ and $t\in[0,T]$,
...
$\bfz=\bf0$, recall that the balance of angular momentum is
$$\label{eq:angularmomentum}
  \frac{d}{dt}\int_{\Omega_t}\bfy\times (\rho\bfv)\dVy =\int_{\partial\Omega_t}\bfy\times \bft \dAy+\int_{\Omega_t}\bfy\times \rho\bfb\dVy,$$
...
$$\DDt{}\left(\bfy\times\bfv\right) = \DDt{\bfy}\times\bfv+\bfy\times\DDt{\bfv} =\underbrace{\bfv\times\bfv}_{=\bf0}+ \bfy\times\DDt{\bfv}.$$
Moreover,
Proposition [\[prop:TimeDerivativeMassIntegrals\]](#prop:TimeDerivativeMassIntegrals){reference-type="ref"
reference="prop:TimeDerivativeMassIntegrals"} entails that
$$\begin{aligned}
...
[\[eq:angularmomentum\]](#eq:angularmomentum){reference-type="eqref"
reference="eq:angularmomentum"} and using the fact that $\bft=\bfS\bfn$,
we obtain
...
Now, using
Proposition [\[prop:LinearMomentumEulerian\]](#prop:LinearMomentumEulerian){reference-type="ref"
reference="prop:LinearMomentumEulerian"} to substitute for
...
Proposition [\[prop:staticequilibriumeqns\]](#prop:staticequilibriumeqns){reference-type="ref"
reference="prop:staticequilibriumeqns"}, we have:
...
$$\frac{d}{dt}K[\Omega_t]+\int_{\Omega_t} \bfS:\bfL \dVy = P[\Omega_t]\quad\text{for all }t\in[0,T],$$
...
Axiom [\[ax:1stLaw\]](#ax:1stLaw){reference-type="ref"
reference="ax:1stLaw"} to deduce that the balance law for energy in
$\Omega_t\subseteq B_t$ is
$$\frac{d}{dt}\int_{\Omega_t} \rho\phi\dVy = \int_{\Omega_t}\bfS:\bfL\dVy -\int_{\partial \Omega_t}\bfq\cdot\bfn\dAy +\int_{\Omega_t}\rho r\dVy.$$
Recall that:
...
We can apply
Proposition [\[prop:TimeDerivativeMassIntegrals\]](#prop:TimeDerivativeMassIntegrals){reference-type="ref"
reference="prop:TimeDerivativeMassIntegrals"} along with the divergence
...
Axiom [\[ax:1stLaw\]](#ax:1stLaw){reference-type="ref"
reference="ax:1stLaw"}, the First Law of Thermodynamics, entails that
...
$$\frac{d}{dt}\int_{\Omega_t} \rho\eta\dVy \geq \int_{\Omega_t}\frac{\rho r}{\theta}\dVy-\int_{\partial\Omega_t}\frac{\bfq\cdot\bfn}{\theta}\dAy,$$
where:
...
::: proposition
If $\bfphi:B\times[0,T]\to\bbE^3$ is a motion, then
Axiom [\[ax:2ndLaw\]](#ax:2ndLaw){reference-type="ref"
reference="ax:2ndLaw"}, i.e. the Clausius-Duhem inequality, entails that
...
we recall is positive), we obtain $$\label{eq:C-DEulerian}
...
$$\delta = \theta\rho\DDt\eta-(\rho r-\nabla_\bfy\cdot\bfq),$$ which
allows us to rewrite
[\[eq:C-DEulerian\]](#eq:C-DEulerian){reference-type="eqref"
reference="eq:C-DEulerian"} as $$\label{eq:C-DEulerian2}
...
$\nabla\theta=\bf0$, then
[\[eq:C-DEulerian2\]](#eq:C-DEulerian2){reference-type="eqref"
reference="eq:C-DEulerian2"} implies that $\delta\geq0$, i.e. the
...
$$\begin{aligned}
    ...
    q_i(\bfy,t) &&&\text{3 components of heat flux,}\\
    \phi(\bfy,t) &&&\text{1 internal energy field per unit mass, and}\\
    \eta(\bfy,t) &&&\text{1 entropy field per unit mass.}
...
-   Since the number of unknowns is greater than the number of
    ...
    $(\bfS,\bfq,\phi,\eta)$ to the fields $(\rho,\bfv,\theta)$. Such
...
the arguments applied in
Section [1.3.1](#sec:EulerianMass){reference-type="ref"
reference="sec:EulerianMass"}:

::: proposition
[]{#prop:MassLagrangian label="prop:MassLagrangian"} If
$\bfphi:B\times[0,T]\to\bbE^3$ is a motion with associated deformation
...
We recall from
Axiom [\[ax:LawsOfInertia\]](#ax:LawsOfInertia){reference-type="ref"
reference="ax:LawsOfInertia"} that the balance of linear momentum for
$\Omega_t\subseteq B_t$ entails that
$$\frac{d}{dt}\bfl[\Omega_t] = \bfr[\Omega_t].$$ In order to derive PDEs
in material coordinates, we therefore need to express the quantities
$\bfl[\Omega_t]$ and $\bfr[\Omega_t]$ in terms of those same material
coordinates.

Changing variable, and recalling that $\Omega_t=\bfphi(\Omega,t)$, and
applying
Proposition [\[prop:MassLagrangian\]](#prop:MassLagrangian){reference-type="ref"
reference="prop:MassLagrangian"}, we find $$\begin{aligned}
  \bfl[\Omega_t]
...
\end{aligned}$$ A similar argument for $\bfr[\Omega_t]$ yields
$$\begin{aligned}
  \bfr[\Omega_t]
...
Proposition [\[prop:MassLagrangian\]](#prop:MassLagrangian){reference-type="ref"
reference="prop:MassLagrangian"} to write $$\bfr[\Omega_t]
...
$$\frac{d}{dt}\int_\Omega \rho_0(\bfx)\ppt{\bfphi}(\bfx,t)\dVx
  = \int_{\partial \Omega} \bfP(\bfx,t)\bfnu(\bfx)\dAx+\int_{\Omega_t}\rho_0(\bfx)\bfb_m(\bfx,t)\dVx.$$
...
$$\int_\Omega \rho_0\frac{\partial^2\bfphi}{\partial t^2}\dVx
  = \int_\Omega \nabla_\bfx\cdot \bfP+\rho_0\bfb_m\dVx.$$ Now, since
this holds for all $\Omega\subset B$ (as $\bfphi$ is a bijection), we
...
applying
Proposition [\[prop:MassLagrangian\]](#prop:MassLagrangian){reference-type="ref"
reference="prop:MassLagrangian"} and a change of variable, transforms to
...
$\rho_0\frac{\partial^2\bfphi}{\partial t^2}$ using
Proposition [\[prop:LinearMomentumEulerian\]](#prop:LinearMomentumEulerian){reference-type="ref"
reference="prop:LinearMomentumEulerian"}, we have
$$\frac{d}{dt}K[\Omega_t] = \int_{\Omega}\rho_0\ppt\bfphi\cdot\frac{\partial^2\bfphi}{\partial t^2}\dVx =
...
$$\frac{d}{dt}K[\Omega_t] +\int_\Omega \bfP:\ppt\bfF\dVx = P[\Omega_t]$$
...
Axiom [\[ax:1stLaw\]](#ax:1stLaw){reference-type="ref"
reference="ax:1stLaw"}, the First Law of Thermodynamics, states that
...
are 18 unknown fields: $$\begin{aligned}
    ...
    \kappa_i(\bfx,t) &&&\text{3 components of heat flux,}\\
    \phi_m(\bfx,t) &&&\text{1 internal energy field per unit mass, and}\\
    \eta_m(\bfx,t) &&&\text{1 entropy field per unit mass.}
...
::: axiom
[]{#ax:FrameIndifference label="ax:FrameIndifference"} The spatial mass
density $\rho$, temperature $\theta$, entropy per unit mass $\eta$,
...
stress as functions $$\phi = \widehat{\phi}(\rho,\theta,\bfL),\quad
  \bfq = \widehat{\bfq}(\rho,\theta,\bfL),\quad\text{and}\quad
  \bfS = \widehat{\bfS}(\rho,\theta,\bfL),$$ (where all of the fields
involved are evaluated at spatial point $\bfy$ and time $t$), then in
order to comply with
Axiom [\[ax:FrameIndifference\]](#ax:FrameIndifference){reference-type="ref"
reference="ax:FrameIndifference"}, we must have $$\begin{aligned}
  \widehat{\phi}(\rho^*,\theta^*,\bfL^*) &= \widehat{\phi}(\rho,\theta,\bfL),\\
  \widehat{\bfq}(\rho^*,\theta^*,\bfL^*) &= \bfQ\widehat{\bfq}(\rho,\theta,\bfL),\\
  \widehat{\bfS}(\rho^*,\theta^*,\bfL^*) &= \bfQ\widehat{\bfS}(\rho,\theta,\bfL)\bfQ^T.
\end{aligned}$$ Substituting for the starred fields on the LHS, we have
$$\begin{aligned}
  \widehat{\phi}(\rho,\theta,\bfQ\bfL\bfQ^T) &= \widehat{\phi}(\rho,\theta,\bfL),\\
  \widehat{\bfq}(\rho,\theta,\bfQ\bfL\bfQ^T) &= \bfQ\widehat{\bfq}(\rho,\theta,\bfL),\\
  \widehat{\bfS}(\rho,\theta,\bfQ\bfL\bfQ^T) &= \bfQ\widehat{\bfS}(\rho,\theta,\bfL)\bfQ^T,
\end{aligned}$$ which must hold for all rotations $\bfQ$ and all
admissible values of $\rho, \theta$ and $\bfL$.
