...
$$
\frac{1}{\area(\partial\Omega)}\int_{\partial\Omega} \bft(\bfn(\bfx),\bfx)\dAx\to\bf0\quad\text{as}\quad\vol(\Omega)\to0.
$$ {#eq-arcondition1}
...
$$\lim_{\delta\to0}\frac{1}{\area(\partial T_\delta)}\int_{\partial T_\delta} \bft(\widehat{\bfn}(\bfx'),\bfx')\,dA_{\bfx'} = \bf0,$$
...
each of the faces, we have $$
    \lim_{\delta\to0}\frac{1}{\area(\partial T_\delta)}\bigg(\int_{\Gamma_{\delta,n}} \bft(\bfn,\bfx')\,dA_{\bfx'}+\sum_{i=1}^3\int_{\Gamma_{\delta,i}} \bft(-\bfe_i,\bfx')\,dA_{\bfx'}\bigg) = \bf0.$$ {#eq-tetlimit}
...
$\bfx'_{\delta,n}\in\Gamma_{\delta,n}$ such that $$\begin{gathered}
...
\end{gathered}$$ We note that as $\delta\to0$, we have that
$\bfx'_{\delta,n}\to\bfx$ and $\bfx'_{\delta,i}\to\bfx$, since the faces
shrink to the point $\bfx$.

Using these facts and
@eq-tetarearelation1, we have $$\begin{gathered}
...
$$\bft(\bfn,\bfx)+\sum_{i=1}^3n_i\bft(-\bfe_i,\bfx) = \bf0.$$ Applying
...
$$
  \bfr(\Omega) = \bfr_b(\Omega)+\bfr_s(\partial\Omega) = \int_\Omega \rho(\bfx)\bfb(\bfx)\dVx+\int_{\partial\Omega}\bft(\bfx)\dAx,$$ {#eq-resultantforce}
and the resultant torque on $\Omega$ about the point $\bfz\in\bbE^3$ is
$$
  \begin{aligned}
    \bftau(\Omega;\bfz)
    &= \bftau_b(\Omega;\bfz)+\bftau_s(\partial\Omega;\bfz)\\
    &=\int_\Omega (\bfx-\bfz)\times\rho(\bfx)\bfb(\bfx)\dVx+\int_{\partial \Omega}(\bfx-\bfx)\times\bft(\bfx)\dAx.
  \end{aligned}
  $$ {#eq-resultanttorque} At a static equilibrium, we assume that both the
resultant force and torque for any $\Omega$ vanish, which is encoded in
the following axiom.

::: axiom
[]{#ax:staticequilibrium label="ax:staticequilibrium"} If a body $B$ is
...
$$
    \begin{gathered}
      \bfr(\Omega)=\int_\Omega \rho(\bfx)\bfb(\bfx)\dVx+\int_{\partial\Omega}\bft(\bfx)\dAx =\bf0\\
      \bftau(\Omega;\bfz) = \int_\Omega (\bfx-\bfz)\times\rho(\bfx)\bfb(\bfx)\dVx+\int_{\partial \Omega}(\bfx-\bfz)\times\bft(\bfx)\dAx =\bf0
    \end{gathered}$${#eq-equilibriumconditionsINT} for any $\Omega\subseteq B$.
:::
...
We now use
Axiom [\[ax:staticequilibrium\]](#ax:staticequilibrium){reference-type="ref"
reference="ax:staticequilibrium"} to derive differential equations which
...
field $\bfb$ are continuous, then the equilibrium conditions
[\[eq:equilibriumconditionsINT\]](#eq:equilibriumconditionsINT){reference-type="eqref"
reference="eq:equilibriumconditionsINT"} are equivalent to
$$\label{eq:equilibriumequationsDE}
    \begin{gathered}
      (\nabla\cdot\bfS)(\bfx)+\rho(\bfx)\bfb(\bfx) = \bf0 \\
      \bfS^T(\bfx)=\bfS(\bfx)
    \end{gathered}$$ for any $\bfx\in B$. In components, these equations
are: $$\begin{gathered}
...
[\[eq:equilibriumequationsDE\]](#eq:equilibriumequationsDE){reference-type="eqref"
reference="eq:equilibriumequationsDE"} assuming that
Axiom [\[ax:staticequilibrium\]](#ax:staticequilibrium){reference-type="ref"
reference="ax:staticequilibrium"} holds, we first use the definition of
$\bfS$ to write the first equation in
[\[eq:equilibriumconditionsINT\]](#eq:equilibriumconditionsINT){reference-type="eqref"
reference="eq:equilibriumconditionsINT"} as
$$\int_{\partial \Omega}\bfS\bfn\dAx+\int_\Omega\rho\bfb\dVx=\bf0.$$
Applying the Tensor Divergence Theorem
@prp-tensordivthm to the first integral, we obtain
$$\int_\Omega \big(\nabla\cdot\bfS+\rho\bfb\big)\dVx=\bf0.$$ Since this
equation hold for an arbitrary open set $\Omega\subseteq B$, the
Localisation Theorem
(@prp-localisation) allows us to conclude that the integrand
vanishes, which is exactly the first result.

To establish the second result, we write the second equation in
[\[eq:equilibriumconditionsINT\]](#eq:equilibriumconditionsINT){reference-type="eqref"
reference="eq:equilibriumconditionsINT"} as
$$\int_{\partial\Omega} (\bfx-\bfz)\times (\bfS\bfn)\dAx+\int_\Omega (\bfx-\bfz)\times\rho\bfb\dVx=\bf0.$$
Since we have just shown $\rho\bfb=-\nabla\cdot\bfS$, we can substitute,
and rewrite $$
    \int_{\partial\Omega} (\bfx-\bfz)\times (\bfS\bfn)\dAx-\int_\Omega (\bfx-\bfz)\times\big(\nabla\cdot\bfS\big)\dVx=\bf0.$$
    {#eq-equilibriumproof1}
Next, note that we may define the tensor
$\bfR=R_{il}\bfe_i\otimes\bfe_l$ to be
$$R_{il} = \epsilon_{ijk}(x_j-z_j)S_{kl},$$ which has the property that
$\bfR\bfn=(\bfx-\bfz)\times(\bfS\bfn)$, and hence
[\[eq:equilibriumproof1\]](#eq:equilibriumproof1){reference-type="eqref"
reference="eq:equilibriumproof1"} can be written as
$$\int_{\partial\Omega}\bfR\bfn\dAx-\int_\Omega (\bfx-\bfz)\times(\nabla\cdot \bfS)\dVx=\bf0.$$
...
\end{aligned}$$ Considering each of these equations for $i=1,2,3$, we
see that
...
second equation in
[\[eq:equilibriumequationsDE\]](#eq:equilibriumequationsDE){reference-type="eqref"
reference="eq:equilibriumequationsDE"}.

To show that
[\[eq:equilibriumconditionsINT\]](#eq:equilibriumconditionsINT){reference-type="eqref"
reference="eq:equilibriumconditionsINT"} hold starting from
[\[eq:equilibriumequationsDE\]](#eq:equilibriumequationsDE){reference-type="eqref"
reference="eq:equilibriumequationsDE"}, we need simply reverse the
arguments above.
:::
...
2.  The local equilibrium equations
    [\[eq:equilibriumequationsDE\]](#eq:equilibriumequationsDE){reference-type="eqref"
    reference="eq:equilibriumequationsDE"} do not completely determine
...
$$\bfS = -\pi\bfI,$$ where $\pi$ is a scalar, we say that a *spherical*
